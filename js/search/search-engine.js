(function (root, factory) {
    const classifier = typeof module === 'object' && module.exports ? require('./biomarker-classifier.js') : (root.ClinicalTrialApp && root.ClinicalTrialApp.search) || {};
    const api = factory(classifier);
    if (typeof module === 'object' && module.exports) module.exports = api;
    if (root) { root.ClinicalTrialApp = root.ClinicalTrialApp || {}; root.ClinicalTrialApp.search = Object.assign(root.ClinicalTrialApp.search || {}, api); }
    ...
    const parseSearchQuery = (query) => {
        const raw = classifier.normalizeUnicode(query).trim(); let remaining = raw; const filters = [];
        const definitions = [
        ...
        for (const definition of definitions) { if (definition.regex.test(remaining)) { filters.push({ type: 'biomarker', marker: 'HER2', status: definition.status }); remaining = remaining.replace(definition.regex, ' '); break; } }
    ...
    const trialFields = (trial) => [
        ...
        { name: 'cancerType', value: [trial && trial.cancerType, Array.isArray(trial && trial.cancerTypes) ? trial.cancerTypes.map((item) => `${item.type || ''} ${(item.lines || []).join(' ')}`).join(' ') : ''].join(' '), weight: 65 },
        ...
        { name: 'pi', value: trial && ((trial.contacts && trial.contacts.pi) || trial.pi), weight: 54 },
        ...
        { name: 'contactRaw', value: trial && trial.contacts && trial.contacts.raw, weight: 44 },
    ...
    const scoreFreeText = (trial, freeText) => {
        ...
        const terms = normalize(freeText).split(' ').filter(Boolean), reasons = []; let score = 0;
        for (const term of terms) {
            const compactTerm = normalizeCompact(term); let termMatched = false;
            for (const field of trialFields(trial)) {
                const normalizedValue = normalize(field.value), compactValue = normalizeCompact(field.value);
                if (!normalizedValue) continue;
                if (normalizedValue.includes(term) || (compactTerm && compactValue.includes(compactTerm))) {
                    termMatched = true; let fieldScore = field.weight; if (field.name === 'code' && compactValue === compactTerm) fieldScore += 100;
                    score += fieldScore; reasons.push({ type: 'text', field: field.name, term, score: fieldScore }); break;
                }
            }
    ...
    const evaluateTrialSearch = (trial, query) => {
        const parsed = typeof query === 'string' ? parseSearchQuery(query) : query, reasons = []; let score = 0;
        for (const filter of parsed.filters || []) {
            if (filter.type === 'biomarker' && filter.marker === 'HER2') {
                ...
                score += filter.status === 'anyMention' ? 30 : 90; reasons.push({ type: 'biomarker', marker: 'HER2', status: filter.status, summary: classification.summary, evidence: classification.mentions.slice(0, 3) });
            }
        }
        const freeTextResult = scoreFreeText(trial, parsed.freeText || ''); if (!freeTextResult.matched) return { matched: false, score: 0, reasons: [], parsed };
        score += freeTextResult.score; reasons.push(...freeTextResult.reasons); return { matched: true, score, reasons, parsed };
    };
    const searchTrials = (trials, query) => (Array.isArray(trials) ? trials : []).map((trial, index) => ({ trial, index, result: evaluateTrialSearch(trial, query) })).filter((item) => item.result.matched).sort((a, b) => b.result.score - a.result.score || a.index - b.index);
